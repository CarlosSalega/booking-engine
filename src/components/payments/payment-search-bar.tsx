/**
 * `PaymentSearchBar` — debounced text search for the payments list.
 *
 * Updates the URL's `?search=...` parameter via `router.push`, which
 * triggers a Server Component re-render of the payments page. The
 * debounce keeps the URL from updating on every keystroke.
 *
 * Mirrors the patients `PatientSearchBar` and the professionals
 * `ProfessionalSearchBar` pattern: source of truth is the URL, we
 * mirror it into local state for input control, and a 300ms debounce
 * commits the change. The data layer (`getPayments`) does a
 * case-insensitive `contains` match against the linked booking's
 * patient name OR professional name, so a single text input covers
 * both fields without an explicit "search by" selector.
 *
 * UX: the input shows the current `?search=` value. Pressing Enter
 * or waiting 300ms after the last keystroke commits the value to
 * the URL. The "Limpiar" button resets the search.
 */
