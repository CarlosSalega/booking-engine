/**
 * `ProfessionalSearchBar` — debounced text search for the
 * professionals list.
 *
 * Updates the URL's `?search=...` parameter via `router.push`, which
 * triggers a Server Component re-render of the professionals page. The
 * debounce keeps the URL from updating on every keystroke.
 *
 * Mirrors the services `ServiceSearchBar` and patients
 * `PatientSearchBar` pattern: source of truth is the URL, we mirror
 * it into local state for input control, and a 300ms debounce commits
 * the change. The data layer (`getProfessionals`) does a
 * case-insensitive `contains` match against the linked user's
 * `name` AND `email`, so a single text input covers both fields
 * without an explicit "search by" selector.
 *
