/**
 * Tests for the `completeBooking` Server Action.
 *
 * Same pattern as `confirmBooking` (R+G combined per task 2.10 spec).
 * The action transitions CONFIRMED → COMPLETED (terminal).
 */
...
const { completeBooking } = await import("../complete-booking.action");
...
describe("completeBooking", () => {
  ...
  it("completes a CONFIRMED booking when called by ADMIN", async () => {
    getSessionMock.mockResolvedValueOnce(sessionFor("ADMIN"));

    const result = await completeBooking({ bookingId: BOOKING_ID });
  ...
  it("calls revalidatePath('/dashboard/bookings') after a successful complete", async () => {
    getSessionMock.mockResolvedValueOnce(sessionFor("ADMIN"));

    await completeBooking({ bookingId: BOOKING_ID });
  ...
  it("rejects PROFESSIONAL completing a booking that is not theirs", async () => {
    prismaMock.booking.findFirst.mockResolvedValueOnce(
